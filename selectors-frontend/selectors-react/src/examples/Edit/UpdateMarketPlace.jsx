  ...
  const handlesubmit = async (e) => {
    ...
    try {
      let result = await axios.patch(
        `http://localhost:5000/api/v1/selectors/updatemarketplace/${id}`,
        {
    ...
    const fetchData = async () => {
      ...
      try {
        const result = await axios.get(
          `http://localhost:5000/api/v1/selectors/findmarketplace/${id}`
        );
        dispatch({ type: "FETCH_SUCCESS", payload: result.data });
        newURL(result.data.marketplaceUrl);
        newMarketplace(result.data.marketplace);
        console.log(result);
      } catch (error) {
